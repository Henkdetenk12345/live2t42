#!/usr/bin/env python3
"""
Live Digtal Teletext to T42 converter
Direct combination of @ZXGuesser ts2pes.py and pes2t42.py
"""

import sys
import socket
import threading
import queue
import time
import requests

# Bit reversal lookup table
REVERSE_BYTES = []


clientQueues = []

class PacketServerClient:
    @staticmethod
    def clientConnected(clientsocket, addr, q):
        clientQueues.append(q)
        print(f"Client connected: {addr}", file=sys.stderr)
        while True:
            try:
                clientsocket.sendall(q.get())
            except OSError:
                break
        clientQueues.remove(q)
        clientsocket.close()
        print(f"Client disconnected: {addr}", file=sys.stderr)


def _run_tcp_server(port, family, bind_addr):
    sock = socket.socket(family, socket.SOCK_STREAM)
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_SNDBUF, 672)
    if family == socket.AF_INET6:
        try:
            sock.setsockopt(socket.IPPROTO_IPV6, socket.IPV6_V6ONLY, 1)
        except AttributeError:
            pass
    sock.setsockopt(socket.SOL_SOCKET, socket.SO_REUSEADDR, 1)
    sock.bind((bind_addr, port))
    sock.listen()
    while True:
        conn, addr = sock.accept()
        q = queue.Queue()
        t = threading.Thread(target=PacketServerClient.clientConnected, args=(conn, addr, q))
        t.daemon = True
        t.start()
    sock.close()


def start_packet_server(port):
    """Start IPv4 and IPv6 TCP listener threads."""
    t4 = threading.Thread(target=_run_tcp_server, args=(port, socket.AF_INET, "0.0.0.0"))
    t4.daemon = True
    t4.start()

    t6 = threading.Thread(target=_run_tcp_server, args=(port, socket.AF_INET6, "::"))
    t6.daemon = True
    t6.start()

    print(f"Packet server listening on port {port} (IPv4 + IPv6)", file=sys.stderr)


def broadcast_field(data: bytes):
    """Send a field to all connected packet-server clients."""
    for q in clientQueues[:]:
        if q.qsize() > 4:          # max 4 fields buffering, same as packet-server.py
            with q.mutex:
                q.queue.clear()
        q.put(data)


class LiveTeletextConverter:
    def __init__(self, stream_url, teletext_pid, packet_server_port=None):
        self.stream_url = stream_url
        self.teletext_pid = teletext_pid
        self.packet_server_port = packet_server_port

        # State from pes2t42.py
        self.line_offset = 0
        self.field_parity = 1
        self.fieldbytes = bytearray(42 * 16)

    def process_pes_data(self, pes_data):
        """Process PES payload data - direct from pes2t42.py logic"""
        outputs = []

        if len(pes_data) < 9:
            return outputs

        prefix = int.from_bytes(pes_data[0:3], "big")
        if prefix != 1:
            print("Missing PES header. Skipping", file=sys.stderr)
            return outputs

        if pes_data[3] != 0xBD:
            print("Invalid stream ID", file=sys.stderr)
            return outputs

        length = int.from_bytes(pes_data[4:6], "big")
        headerlength = pes_data[8]

        data_start = 9 + headerlength
        data_end = 6 + length if length > 0 else len(pes_data)

        if data_end > len(pes_data):
            return outputs

        packet = pes_data[data_start:data_end]

        if len(packet) < 1:
            return outputs

        if packet[0] < 0x10 or packet[0] > 0x1F:
            return outputs

        for i in range(1, len(packet), 0x2E):
            if i + 0x2E > len(packet):
                break

            if packet[i] == 0x02 or packet[i] == 0x03:
                datafield = packet[i+2:i+0x2E]

                if len(datafield) < 0x2C:
                    continue

                fp = (datafield[0] >> 5) & 1
                lo = datafield[0] & 0x1F

                if lo < 0x07 or lo > 0x16:
                    print(f"Invalid line offset {lo}", file=sys.stderr)
                    continue

                if fp != self.field_parity:
                    outputs.append(bytes(self.fieldbytes))
                    self.field_parity = fp
                    self.fieldbytes = bytearray(42 * 16)
                    self.line_offset = 0

                if lo < self.line_offset:
                    print(f"Line offset decreased from {self.line_offset} to {lo}", file=sys.stderr)

                for j in range(0x2A):
                    self.fieldbytes[(lo - 7) * 0x2A + j] = REVERSE_BYTES[datafield[2 + j]]

                self.line_offset = lo

        return outputs

    def run(self):
        """Main streaming loop"""
        print(f"Connecting to {self.stream_url}", file=sys.stderr)
        print(f"Filtering teletext PID: 0x{self.teletext_pid:04x}", file=sys.stderr)

        if self.packet_server_port:
            start_packet_server(self.packet_server_port)
            print(f"Output: T42 stream via packet server on port {self.packet_server_port}", file=sys.stderr)
        else:
            print(f"Output: T42 stream to stdout", file=sys.stderr)

        try:
            output = None if self.packet_server_port else sys.stdout.buffer

            with requests.get(self.stream_url, stream=True, timeout=10) as response:
                response.raise_for_status()

                field_count = 0
                pes_buffer = bytearray()
                in_pes = False

                for chunk in response.iter_content(chunk_size=188 * 100):
                    for i in range(0, len(chunk), 188):
                        if i + 188 > len(chunk):
                            break

                        packet = chunk[i:i+188]

                        if len(packet) != 188:
                            continue

                        if packet[0] != 0x47:
                            print("Packet without sync byte", file=sys.stderr)
                            continue

                        PID = ((packet[1] & 0x1F) << 8) | packet[2]

                        if PID != self.teletext_pid:
                            continue

                        PUSI = (packet[1] >> 6) & 1
                        Adaption = (packet[3] >> 4) & 3

                        payload = None
                        if Adaption == 1:
                            payload = packet[4:188]
                        elif Adaption == 3:
                            payload = packet[5 + packet[4]:188]

                        if payload is None:
                            continue

                        if PUSI:
                            if len(pes_buffer) > 0:
                                fields = self.process_pes_data(bytes(pes_buffer))
                                for field in fields:
                                    if self.packet_server_port:
                                        broadcast_field(field)
                                    else:
                                        output.write(field)
                                        output.flush()
                                    field_count += 1
                                    if field_count % 50 == 0:
                                        print(f"Fields: {field_count}", file=sys.stderr)

                            pes_buffer = bytearray(payload)
                            in_pes = True
                        elif in_pes:
                            pes_buffer.extend(payload)

        except KeyboardInterrupt:
            print("\nStopped by user", file=sys.stderr)
        except Exception as e:
            print(f"Error: {e}", file=sys.stderr)
            import traceback
            traceback.print_exc()
            sys.exit(1)


def main():
    if len(sys.argv) < 3:
        print("Usage: python3 live2t42.py <stream_url> <pid_hex> [--packetserver <port>]", file=sys.stderr)
        print("Example (stdout):       python3 live2t42.py http://192.168.1.9:9981/stream/... 0835 | vbit-iv.py 1 0", file=sys.stderr)
        print("Example (packetserver): python3 live2t42.py http://192.168.1.9:9981/stream/... 0835 --packetserver 5431", file=sys.stderr)
        sys.exit(1)

    stream_url = sys.argv[1]
    pid_hex = sys.argv[2]

    try:
        teletext_pid = int(pid_hex, 16)
    except ValueError:
        print(f"Error: Invalid PID '{pid_hex}'. Must be hexadecimal.", file=sys.stderr)
        sys.exit(1)

    # Parse optional --packetserver <port>
    packet_server_port = None
    args = sys.argv[3:]
    if "--packetserver" in args:
        idx = args.index("--packetserver")
        if idx + 1 >= len(args):
            print("Error: --packetserver requires a port number.", file=sys.stderr)
            sys.exit(1)
        try:
            packet_server_port = int(args[idx + 1])
            if packet_server_port <= 1024 or packet_server_port > 65535:
                raise ValueError
        except ValueError:
            print("Error: --packetserver port must be an integer between 1025 and 65535.", file=sys.stderr)
            sys.exit(1)

    converter = LiveTeletextConverter(stream_url, teletext_pid, packet_server_port)
    converter.run()

if __name__ == "__main__":
    main()
